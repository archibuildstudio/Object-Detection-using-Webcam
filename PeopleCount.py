...
def main():
    ...
    while True:
        ...
        # Filter detections (class_id == 0)
        persons = detections[detections.class_id == 0]  # Class 0 is for "person"

        # Filter detections in the zone
        detections_in_zone = persons[zone.trigger(detections=persons)]
        ...
        # Count persons
        person_count = len(detections_in_zone)

        # Display count on frame
        cv2.putText(frame, f"Person: {person_count}", (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
